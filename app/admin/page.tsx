...
function DashboardContent({ receipts, events, posSales, drinkSales, onRefresh }: { receipts: ReceiptRecord[]; events: AdminEvent[]; posSales?: any; drinkSales?: any; onRefresh: () => void }) {
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-4">
            ...
            <div className="flex items-center gap-4 text-right">
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-zinc-400">Total Recaudado en Barra</p>
                <p className="text-2xl font-black text-amber-400">${drinkSales?.totalRevenue || 0}.00</p>
              </div>
              <div className="h-8 w-px bg-white/10" />
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-zinc-400">Bebidas Vendidas</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
            <div className="flex items-center justify-between rounded-2xl border border-amber-500/20 bg-amber-950/20 p-3.5">
              <div>
                <span className="text-[8px] font-black uppercase tracking-widest text-amber-400">Cocteles Especiales</span>
                <p className="text-sm font-black text-white mt-0.5">
                  {Object.values(drinkSales?.items || {}).filter((i: any) => i.category === "Cocteles Especiales").reduce((sum: number, item: any) => sum + (item.count || 0), 0)} uds.
                </p>
              </div>
              <span className="text-sm font-black text-amber-300 font-bold">
                ${Object.values(drinkSales?.items || {}).filter((i: any) => i.category === "Cocteles Especiales").reduce((sum: number, item: any) => sum + (item.revenue || 0), 0)}
              </span>
            </div>

            <div className="flex items-center justify-between rounded-2xl border border-orange-500/20 bg-orange-950/20 p-3.5">
              <div>
                <span className="text-[8px] font-black uppercase tracking-widest text-orange-400">Botellas de Licor</span>
                <p className="text-sm font-black text-white mt-0.5">
                  {Object.values(drinkSales?.items || {}).filter((i: any) => i.category === "Botellas").reduce((sum: number, item: any) => sum + (item.count || 0), 0)} uds.
                </p>
              </div>
              <span className="text-sm font-black text-orange-300 font-bold">
                ${Object.values(drinkSales?.items || {}).filter((i: any) => i.category === "Botellas").reduce((sum: number, item: any) => sum + (item.revenue || 0), 0)}
              </span>
            </div>
          </div>
        </motion.div>
...
function AdminDashboardInner({ onLogout, onOpenDesigner }: { onLogout: () => void; onOpenDesigner: () => void }) {
  ...
  const [drinkSales, setDrinkSales] = useState<any>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [receiptRes, eventRes, posRes, drinkRes] = await Promise.all([
        fetch("/api/access-drop/receipts"),
        fetch("/api/admin/events"),
        fetch("/api/pos/sales").catch(() => null),
        fetch("/api/pos/drinks").catch(() => null),
      ]);
  ...
  const renderContent = () => {
    switch (activeSection) {
      case "dashboard":
        return <DashboardContent receipts={receipts} events={events} posSales={posSales} drinkSales={drinkSales} onRefresh={loadData} />;
      case "entradas":
        return (
          <div className="space-y-5 p-4 md:p-6">
      ...
      case "ventas":
        return <VentasSection receipts={receipts} posSales={posSales} drinkSales={drinkSales} onRefreshPosSales={loadData} />;
      case "clientes":
