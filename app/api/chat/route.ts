...
const replies: [RegExp, string][] = [
  [/^(hola|hey|buenas|que tal|q tal|hi|buenos|saludos|buen[oa])\b/i,
    "Hola. Bienvenido a DAWGS. Pregunta lo que necesites: entradas, precio, fecha, giveaway, ropa, estudio, o lo que sea."],
  ...
  [/ropa|wear|merch|streetwear|camiseta|polera|gorra|buso|sudadera|talla|vestimenta|moda/i,
    "DAWGS Wear es nuestra linea de ropa urbana premium. En la seccion Wear puedes ver las colecciones disponibles, tallas y precios. Las piezas son estilo streetwear, edicion limitada."],

  // Studio / música
  [/studio|m[uú]sica|produccion|mezcla|master|beat|grabar|cancion|instrumental|sonido|estudio/i,
    "DAWGS Studio ofrece servicios de produccion musical, mezcla y master. En la seccion Studio puedes iniciar un proyecto directamente por WhatsApp y coordinar con el equipo."],
...
function getReply(text: string): string {
  ...
  if (maxTokens <= 2) {
    return "Puedes preguntarme sobre el evento TRAP LOUD, precio del ticket, fecha, giveaway, ropa DAWGS Wear, estudio de musica, o cualquier otra cosa. Intenta con una pregunta mas completa.";
  }

  return "No tengo una respuesta exacta para eso. Pero dime: te interesa comprar ticket para TRAP LOUD, saber el precio, la fecha, el giveaway, la ropa Wear, o el estudio de musica? Respondo cualquier cosa.";
}
